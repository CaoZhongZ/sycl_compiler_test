...
%.debug : %.o
	...
	file-table-tform -extract=Code -drop-titles -o ./tmp/$<-list.txt ./tmp/$<-list.table
	...
	llvm-foreach --out-ext=out --in-file-list=./tmp/$<-list1.txt --in-replace=$<-list1.txt  --out-file-list=./tmp/$<.out --out-replace=./tmp/$<.out --jobs=8 -- ocloc -output ./tmp/$<.out -file ./tmp/$<-list1.txt -output_no_suffix -spirv_input -device pvc
