...
constexpr int group_size() { return 16 /* Use query interface for sub-group */ * 4; }
constexpr int item_work_size() { return 4; }
constexpr int group_work_size() { return item_work_size() * group_size(); }
...
static OffsetCalculator<N> make_input_offset_calculator(const at::TensorIteratorBase& iter) {
  ...
  assert(N == iter.ntensors() - iter.noutputs());
...
inline void elementwise_kernel_helper(sycl::nd_item<1> pos, func_t f, policy_t policy) {
  using traits = function_traits<func_t>;
  using return_t = typename traits::result_type;
  using args_t = typename traits::ArgsTuple;

  int idx = pos.get_group(0);

  return_t results[item_work_size()];
  args_t args[item_work_size()];

  // load
  policy.load(pos, args, idx);

  // compute
  #pragma unroll (item_work_size())
  for (int i = 0; i < item_work_size(); i++) {
    if (policy.check_inbounds(pos.get_local_id(), i)) {
      results[i] = std::apply(f, args[i]);
    }
  }

  // store
  policy.store(pos, results, idx);
}
...
void gpu_kernel(at::TensorIteratorBase& iter, const func_t& f) {
  if (iter.numel() == 0) {
    return;
  }
...
void opmath_gpu_kernel_with_scalars(at::TensorIteratorBase& iter, const func_t& f) {
...
template <typename func_t>
void gpu_kernel_with_scalars(at::TensorIteratorBase& iter, const func_t& f) {
  using traits = function_traits<func_t>;
  static_assert(
      traits::arity == 2,
      "gpu_kernel_with_scalars only supports two input arguments");
...
struct unrolled_elementwise_kernel_for_multi_outputs {
  unrolled_elementwise_kernel_for_multi_outputs(int N, func_t f, array_t data, inp_calc_t ic, out_calc_t oc)
  : N(N), f(f), data(data), ic(ic), oc(oc) {}

  void operator () (sycl::nd_item<1> pos) const {
    int remaining = N - group_work_size() * pos.get_local_id();
    elementwise_kernel_helper(
        pos, f,
        memory::policies::multi_outputs_unroll<array_t, inp_calc_t, out_calc_t, num_outputs>(
...
static inline void launch_unrolled_kernel_for_multi_outputs(int64_t N, const func_t& f, array_t data, inp_calc_t ic, out_calc_t oc) {
  ...
  queue.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1> ({grid * group_size(), group_size()}),
      unrolled_elementwise_kernel_for_multi_outputs<num_outputs, func_t, array_t, inp_calc_t, out_calc_t>(N, f, data, ic, oc));
  });
...
void gpu_kernel_multiple_outputs_impl(at::TensorIteratorBase& iter, const func_t& f) {
  ...
  static_assert(is_tuple<output_t>::value, "f's return type must be `std::tuple`");
